		...
		<div class="col-sm-6 offset-sm-3">
			<div class="card">
				<div class="card-header">
					<h5 class="text-center"> Basis Aturan Baru</h5>
				</div>
				<div class="card-body">
					...
					<div class="row" v-for="(d, index) in aturan.details">
						<div class="col-md-5">
							<div class="form-group">
								...
								<select class="form-control" v-model='d.param' :key="index" @change="getValue($event, index)">
...
<script>
	export default {
		...
		methods: {
			...
			getValue(e, index) {
				axios.post(window.baseUrl+"/"+"kriteria/details/all", {kriteria_id: e.target.value}).then(response => {
			...
			store(e) {
				axios.post(window.baseUrl+"/"+e.target.attributes.action.value, this.aturan).then(response => {
					this.errors = [];
					...
					this.message = 'Basis Aturan telah ditambahkan.';
